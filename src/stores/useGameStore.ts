...
interface GameState {
  ...
  // === 思维骇入 (抽卡) ===
  generateHackPreview: () => Inscription | null; // 生成抽卡预览，不消耗点数（返回null表示点数不足）
  confirmHackResult: (item: Inscription) => void; // 确认接收：消耗点数并添加物品到背包
  ...
  triggerInscriptions: (
    ...
    damageSource?: { type: "skill" | "question"; baseDamage: number }
  ) => number | void;
  ...
  addDamageIndicator: (
    indicator: Omit<DamageIndicator, "id" | "timestamp">
  ) => void;
  ...
  distributeAIQuestionsToSectors: (
    questions: Question[],
    sourceTitle: string
  ) => void;
  ...
  persist(
    (set, get) => ({
      ...
      observerProfile: {
        ...
        currentInscription: null, // 当前装备的铭文（只能有一个）
        clearedSectors: [],
        entropyStabilized: 0,
      ...
      triggerInscriptions: (trigger, damageSource) => {
        const {
          ...
          entropyEntities,
          addBattleLog,
          inscriptionTriggeredFlags,
        } = get();

        // 检查当前装备的铭文（只有一个）
        // 从INSCRIPTIONS常量中查找完整铭文对象（包含effect函数）
        // 这是因为存储到localStorage的铭文会丢失effect函数
        if (!observerProfile.currentInscription) return damageSource?.baseDamage;
        
        const currentInscriptionId = observerProfile.currentInscription.id;
        const matchingInscription = INSCRIPTIONS.find(
          (inscription) =>
            inscription.id === currentInscriptionId &&
            inscription.trigger === trigger
        );

        if (!matchingInscription) return damageSource?.baseDamage;
        ...
        const context: InscriptionEffectContext = {
          ...
          updateConstructs: (
            updater: (constructs: Construct[]) => Construct[]
          ) => {
          ...
          updateEnemies: (
            ...
            set((state) => ({
              entropyEntities: updater(state.entropyEntities),
            }));
          },
          damageSource,
        };
        ...
        const result = matchingInscription.effect(context);
      ...
      generateHackPreview: () => {
        ...
        console.log(`[思维骇入] 启动骇入，消耗1点，生成: ${selectedItem!.name} (剩余点数: ${get().observerProfile.hackPoints})`);
        return selectedItem!;
      },

      // 确认接收（将物品存入，点数已在启动时消耗）
      confirmHackResult: (item: Inscription) => {
        // 替换当前铭文（只能有一个）
        set((state) => ({
          observerProfile: {
            ...state.observerProfile,
            currentInscription: item, // 替换当前铭文
          },
        }));

        console.log(
          `[思维骇入] 确认接收: ${item.name} - 已替换当前铭文`
        );
      },
      ...
      addHackPoint: () => {
        set((state) => {
          ...
          console.log(
            `[思维骇入] 通关奖励，点数+1 (当前: ${newPoints}/${maxPoints})`
          );
      ...
      addExp: (amount) => {
        ...
        while (currentExp >= currentMaxExp && currentLevel < MAX_LEVEL) {
          ...
          if (currentLevel >= MAX_LEVEL) {
            ...
            console.log(
              `[等级提升] 升至 Lv.${currentLevel}！下一级需要 ${currentMaxExp} EXP`
            );
          }
        }
        ...
        console.log(
          `[经验获得] +${amount} EXP (当前: ${currentExp}/${currentMaxExp})`
        );
      ...
      startBattle: (sectorId) => {
        ...
        if (sector.aiQuestions && sector.aiQuestions.length > 0) {
          // 使用AI生成的题目
          console.log(
            `[AI模式] 扇区 ${sector.name} 使用AI生成的 ${sector.aiQuestions.length} 道题目`
          );
          ...
          if (selectedQuestions.length < totalQuestionsNeeded) {
            const availableSamples = [...SAMPLE_QUESTIONS].filter(
              (q) => !usedIds.has(q.id)
            );
            while (
              selectedQuestions.length < totalQuestionsNeeded &&
              availableSamples.length > 0
            ) {
              const randomIndex = Math.floor(
                Math.random() * availableSamples.length
              );
            ...
            console.warn(
              `[AI模式] AI题目不足，已从样本题库补充 ${
                selectedQuestions.length - sector.aiQuestions.length
              } 道题目`
            );
          }
        } else {
          ...
          if (selectedQuestions.length < totalQuestionsNeeded) {
            console.warn(
              `[样本模式] 题库题目不足！需要 ${totalQuestionsNeeded} 题，但只有 ${selectedQuestions.length} 题可用`
            );
          }
        }
        ...
        const battleConstructs = JSON.parse(
          JSON.stringify(INITIAL_CONSTRUCTS)
        ).map((c: Construct) => ({
          ...c,
          energy: 0,
        ...
        // 深拷贝敌人并应用难度修正
        const battleEnemies = JSON.parse(
          JSON.stringify(sector.entropyEntities)
        ).map((e: EntropyEntity) => ({
          ...e,
        ...
        get().addBattleLog(
          `题目来源: ${isAIMode ? "🤖 AI动态生成" : "📚 内置题库"}`,
          "system"
        );
        get().addBattleLog(
          `本关卡共 ${selectedQuestions.length} 道题目`,
          "system"
        );

        // 如果有铭文，显示铭文信息
        const currentInscription = get().observerProfile.currentInscription;
        if (currentInscription) {
          get().addBattleLog(
            `⬡ 铭文激活: 【${currentInscription.name}】`,
            "system"
          );
        }
        ...
        console.log("[铭文系统] 铭文已消耗，本局战斗结束后将不再存在");
      },
      ...
      useSkill: (constructId, skillId, targetId) => {
        ...
        const previousDeadCount = entropyEntities.filter(
          (e) => e.isDead
        ).length;
        ...
        const newlyDefeatedCount =
          updatedEnemies.filter((e) => e.isDead).length - previousDeadCount;
        ...
        if (updatedEnemies.every((e) => e.isDead)) {
          setTimeout(
            () =>
              set({
                battleState: "VICTORY",
                currentScreen: "CAUSALITY_RECORD",
              }),
            1000
          );
        } else {
      ...
      answerQuestion: (optionIndex) => {
        ...
        const userAnswers = Array.isArray(optionIndex)
          ? optionIndex
          : [optionIndex];
        ...
        if (isCorrect) {
          ...
          let activeAttacker = aliveConstructs.find(
          ...
          let updatedEnemies = entropyEntities.map((e) => {
            ...
            if (e.id === targetId && !e.isDead) {
              ...
              if (newComboCount >= 2) {
                addBattleLog(
                  `连击加成！对 ${targetName} 造成 ${damage} 点伤害！`,
                  "combat"
                );
              } else {
                addBattleLog(
          ...
          updatedEnemies = updatedEnemies.map((e) => {
            if (e.id === "entropy-boss" && !e.isDead) {
              ...
              const burstSkill =
                e.skills?.find(
                  (s) => s.effect.specialEffect === "scaling_damage_by_hp_lost"
                ) ||
                (e.skill?.effect.specialEffect === "scaling_damage_by_hp_lost"
                  ? e.skill
                  : null);
              ...
              if (burstSkill) {
                ...
                for (const threshold of thresholds) {
                  ...
                  // 如果血量低于阈值且未触发过
                  if (
                    currentHpPercent < thresholdRatio &&
              ...
              const recoverySkill = e.skills?.find(
                (s) => s.effect.specialEffect === "heal_once_on_low_hp"
              );

              if (recoverySkill) {
                const currentHpPercent = e.hp / e.maxHp;
                const threshold =
                  (recoverySkill.triggerCondition?.value || 40) / 100;
                const recoveryKey = "boss-entropy-recovery-triggered";

                // 如果血量低于40%且未触发过
                if (
                  currentHpPercent < threshold &&
                  !inscriptionTriggeredFlags.has(recoveryKey)
                ) {
                  ...
                  addBattleLog(
                    `⚠️ 【${e.name}】触发【${
                      recoverySkill.name
                    }】！生命值降至 ${Math.round(currentHpPercent * 100)}%！`,
                    "combat"
                  );
                  addBattleLog(
                    `🔄 逆熵流动！恢复 ${healAmount} 点生命值！`,
                    "system"
                  );
                  addDamageIndicator({
                    value: healAmount,
          ...
          if (targetEnemy?.isDead) {
            ...
            if (nextAlive) {
              ...
              addBattleLog(
                `${targetEnemy.name} 已消解！目标切换至 ${nextAlive.name}`,
                "system"
              );
            }
          }
          ...
          const randomIndex = Math.floor(
            Math.random() * aliveConstructs.length
          );
          ...
          const updatedConstructs = get().constructs.map((c) => {
            ...
            // 检查是否有护盾状态效果
            const shieldEffect = c.statusEffects.find(
              (e) => e.effect === "shield"
            );
            ...
            if (shieldEffect) {
              ...
              // 移除护盾效果（一次性使用）
              newStatusEffects = c.statusEffects.filter(
                (e) => e.effect !== "shield"
              );
            }
          ...
          const latestAttacker = latestEnemies.find(
            (e) => e.id === attacker.id
          );
          ...
          if (skill && skill.triggerCondition?.type === "on_attack") {
            if (skill.currentCooldown === 0) {
              addBattleLog(
                `⚠️ 【${attacker.name}】释放了【${skill.name}】！`,
                "combat"
              );
              ...
              switch (skill.effect.specialEffect) {
                ...
                case "force_cooldown":
                  // 引用消解：随机使一个技能进入冷却
                  {
                    ...
                    if (aliveTargets.length > 0) {
                      ...
                      const target = aliveTargets[targetIdx];
                      const availableSkills = target.skills.filter(
                      ...
                      if (availableSkills.length > 0) {
                        ...
                        const cooldownToAdd =
                          skill.effect.statusToApply?.duration || 3;
                        ...
                        addBattleLog(
                          `🔒 ${target.name} 的【${skillToLock.name}】被强制进入 ${cooldownToAdd} 回合冷却！`,
                          "system"
                        );
                      }
                    }
                ...
                case "energy_drain":
                  // 资源侵蚀：全体每回合损失能量
                  {
                    ...
                    addBattleLog(
                      `💧 全体构造体将在 ${duration} 回合内每回合损失 ${drainValue} 点能量！`,
                      "system"
                    );
                  }
                  break;
                ...
                case "stun_single":
                  // 资源禁锢：随机眩晕一名玩家
                  {
                    ...
                    if (aliveTargets.length > 0) {
                      ...
                      const target = aliveTargets[targetIdx];
                      const stunDuration =
                        skill.effect.statusToApply?.duration || 2;
                      ...
                      addBattleLog(
                        `🔗 ${target.name} 陷入「逻辑死锁」状态，无法行动 ${stunDuration} 回合！`,
                        "system"
                      );
                    }
                  }
                ...
                case "drain_all_energy":
                  // 时序混乱：清空随机一个玩家的能量
                  {
                    ...
                    if (aliveTargets.length > 0) {
                      ...
                      addBattleLog(
                        `⏱️ ${target.name} 的能量被完全清空！（损失 ${drainedEnergy} 点能量）`,
                        "system"
                      );
                    }
                  }
                ...
                case "execute_low_hp":
                  // 系统崩溃：对低血量目标造成双倍伤害
                  {
                    ...
                    const lowHpTargets = currentConstructsAfterSkill.filter(
                      (c) => !c.isDead && c.hp / c.maxHp < threshold
                    );
                      ...
                      currentConstructsAfterSkill =
                        currentConstructsAfterSkill.map((c) => {
                          if (c.id === target.id) {
                            ...
                            addBattleLog(
                              `💀 【处决】${c.name} 血量过低，受到 ${damage} 点致命伤害！`,
                              "combat"
                            );
                            return { ...c, hp: newHp, isDead: newHp <= 0 };
                          }
                ...
                case "extend_cooldowns":
                  // 资源丢失：全体技能冷却+2
                  {
                    const cooldownIncrease =
                      skill.effect.statusToApply?.value || 2;
                    ...
                    addBattleLog(
                      `🔍 404错误！全体冷却中的技能冷却时间 +${cooldownIncrease} 回合！`,
                      "system"
                    );
                  }
                  break;
                ...
                case "heal_on_attack":
                  // 无限迭代：攻击后回血
                  {
                    ...
                    const healAmount = Math.floor(
                      attacker.maxHp * (healPercent / 100)
                    );
                ...
                case "aoe_stun_chance":
                  // 内存越界：AOE伤害 + 概率眩晕
                  {
                    ...
                    const stunDuration =
                      skill.effect.statusToApply?.duration || 1;
              ...
              if (skill.cooldown > 0) {
                addBattleLog(
                  `🔄 【${attacker.name}】的【${skill.name}】冷却中（剩余${skill.currentCooldown}回合）`,
                  "system"
                );
              }
            }
          ...
          // 更新状态
          set({
            constructs: currentConstructsAfterSkill,
            entropyEntities: updatedEnemiesAfterAttack,
          });
      ...
      nextTurn: () => {
        ...
        updatedConstructs = updatedConstructs.map((c) => {
          ...
          // 检查是否有能量侵蚀效果
          const erosionEffect = c.statusEffects.find(
            (e) => e.effect === "entropy_erosion"
          );
          ...
          if (
            erosionEffect &&
            erosionEffect.value > 0 &&
            erosionEffect.value < 100
          ) {
            // 能量侵蚀：每回合损失能量
            newEnergy = Math.max(0, c.energy - erosionEffect.value);
            addBattleLog(
              `⚡ ${c.name} 受到资源侵蚀，损失 ${erosionEffect.value} 点能量！`,
              "combat"
            );
          ...
          // 减少其他状态效果持续时间
          newStatusEffects = newStatusEffects
            .map((e) => ({
              ...e,
        ...
        updatedEnemies = updatedEnemies.map((e) => {
          ...
          // 减少敌人状态效果持续时间
          const newStatusEffects = e.statusEffects
            .map((eff) => ({
              ...eff,
        ...
        if (updatedConstructs.every((c) => c.isDead)) {
          setTimeout(
            () =>
              set({ battleState: "DEFEAT", currentScreen: "CAUSALITY_RECORD" }),
            1000
          );
          return;
        }
        ...
        if (questionQueue.length > 0) {
          ...
          const randomIndex = Math.floor(
            Math.random() * allBattleQuestions.length
          );
      ...
      setSectorAIQuestions: (sectorId, questions, sourceTitle) => {
        ...
        console.log(
          `[AI注入] 扇区 ${sectorId} 已注入 ${questions.length} 道AI题目，来源: ${sourceTitle}`
        );
      },
      ...
      createAISector: (sectorData) => {
        ...
        console.log(
          `[AI创建] 新扇区 "${name}" 已创建，ID: ${sectorId}，包含 ${questions.length} 道题目`
        );
        return sectorId;
      },
      ...
      updateSectorBriefing: (sectorId, briefing) => {
        set((state) => ({
          ...
          currentSector:
            state.currentSector?.id === sectorId
      ...
      distributeAIQuestionsToSectors: (questions, sourceTitle) => {
        ...
        console.log(
          `[AI分配] 开始将 ${totalAvailable} 道题目分配到 ${defaultSectorIds.length} 个默认关卡`
        );
        ...
        const updatedSectors = state.sectors.map((sector) => {
          ...
          console.log(
            `[AI分配] 关卡 "${sector.name}" (${sector.id}): 关联全量题库 ${sectorQuestions.length} 题 (需抽取 ${neededCount} 题)`
          );
        ...
        set({ sectors: updatedSectors });
        console.log(
          `[AI分配] 完成！共分配题目到 ${defaultSectorIds.length} 个关卡`
        );
      },
      ...
      updateSectorMetadata: (sectorUpdates) => {
        ...
        set((state) => ({
          sectors: state.sectors.map((sector) => {
            ...
            if (update) {
              console.log(
                `[扇区更新] ${sector.id}: "${sector.name}" → "${update.name}"`
              );
              return {
                ...sector,
                name: update.name,
        ...
        console.log(
          `[扇区更新] 完成！共更新 ${sectorUpdates.length} 个扇区的名称和描述`
        );
      },
    ...
    }),
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        const state = persistedState as {
          observerProfile?: { level: number; exp: number; maxExp: number };
        };
        ...
        if (version < 2 && state?.observerProfile) {
          ...
          console.log(
            `[数据迁移] 修正 maxExp: ${state.observerProfile.maxExp} -> ${correctMaxExp}`
          );
